...
async function main() {
  ...
  try {
    ...
    // --- users: add trial_dismissed_at ---
    await client.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'users' AND column_name = 'trial_dismissed_at'
        ) THEN
          ALTER TABLE users ADD COLUMN trial_dismissed_at timestamptz;
        END IF;
      END $$;
    `);
    console.log("users: trial_dismissed_at column ready.");
